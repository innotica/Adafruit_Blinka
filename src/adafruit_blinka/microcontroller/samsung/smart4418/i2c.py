...
"""Generic Linux I2C class using smbus2's smbus class"""
...
class I2C:
    ...
    def writeto(self, address, buffer, *, start=0, end=None, stop=True):
        ...
        write_cmd = i2c_msg.write(address, buffer)
        self._i2c_bus.i2c_rdwr(write_cmd)

    def readfrom_into(self, address, buffer, *, start=0, end=None, stop=True):
        """Read data from an address and into the buffer"""
        if end is None:
            end = len(buffer)
        read_cmd = i2c_msg.read(address, end-start)
        self._i2c_bus.i2c_rdwr(read_cmd)
        for i in range(end-start):
            buffer[i + start] = b"\x00"
